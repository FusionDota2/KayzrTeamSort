...
"""
"""
This is the test version that distributes the lowest mmr players that want
to support first.
"""
...
def create_player_dicts(playerfile):
    ...
    with open(playerfile) as infile:
        ...
        for player in players:
            ...
            if player[3] == 'False':
                while player[1] in player_dict.keys() or player[
                    1] in captain_dict.keys():
                    player[1] += 1
                player_dict.update({player[1]: [player[0], player[2]]})
                player_mmr_list.append(player[1])
            else:
                while player[1] in captain_dict.keys() or player[
                    1] in player_dict.keys():
                    player[1] += 1
